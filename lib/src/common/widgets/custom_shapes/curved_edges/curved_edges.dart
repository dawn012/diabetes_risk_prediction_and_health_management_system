...
class TCustomCurvedEdges extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {  // size 表示当前组件（Widget）的宽度和高度，是 Flutter 自动计算并传递 的，而不是手动传入的
    ...
    path.lineTo(0, size.height);  // 从 (0,0) 开始（默认 Path 的起点），绘制一条直线到 (0, size.height)，即 左下角

    // size.height 代表组件的底部（即默认的下边界）
    final firstCurve = Offset(0, size.height - 20);  // size.height - 20 代表比底部 上移 20px（即往上凹陷 20px）。
